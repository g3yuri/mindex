<template>
	<q-page class="q-pa-md q-gutter-md">
		...
		<div class="row">
			<div class="col-6">
				<q-select outlined dense @update:model-value="update_model" v-model="area" label="Area" option-value="id" option-label="label" :options="area_list">
	...
	const props = defineProps({
		base: String
			...
			function drop_click() {
				dle.show = true
				dle.items = [
						...all.value[area.value?.label].filter(e=>e.select)
					]
			}
			function planos_drop(items) {
				helper.api.json(`/sis/pets/drop`,items,b=>{
					_from_api(b)
					dle.show=false
				}, alert)
			}
			...
			const form_fields = computed(() => {
				return [{name:'base', value:'pets'}, {name:'tag', value:area.value?.label}]
			})
