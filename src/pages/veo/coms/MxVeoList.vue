<template>
	<q-page class="q-pa-md q-gutter-md">
		<m-loading ref="main">
			...
			<div class="col-xs-12 col-sm-6" :key="index"
			...
			<div class="col-xs-12 col-sm-6 t-py-2" :key="index"
				v-for="(el, index) in list.tajo">
				<div class="t-flex t-justify-between t-items-center t-border-2 t-rounded-lg t-border-gray-300 t-py-2 t-pl-3">
					<div class="t-flex-1">
						<div class="t-flex t-justify-between t-items-center">
							<div class="t-w-4 t-h-4 t-rounded-lg t-bg-green-400"></div>
							<div class="t-font-bold t-text-xl t-flex-1 t-pl-2">{{el.nivel}} {{el.labor}}</div>
							<div class="t-text-gray-500">{{el.area}}</div>
							<div class="t-text-right">
								<q-icon name="o_chevron_right" size="lg"/>
							</div>
						</div>
						<div class="t-w-full t-flex-1 t-text-gray-500 t-italic">{{el.observacion}}</div>
					</div>
				</div>
			</div>
		</m-loading>
