  ...
  <m-loading ref="main" class="row t-w-full t-flex">
    <div class="col-12 col-sm-6">
      ...
      <v-chart
        ...
        @click="
          (evt) =>
            operador_click(list_area, {
              area: evt.name,
              riesgo: evt.seriesId
            })
        "
      />
      <dash-operador-list ref="list_area" />
    </div>
    <div class="col-12 col-sm-6">
      <div class="t-font-bold t-text-xl t-text-center t-px-2 t-pt-2">
        Fatiga por Cargos
      ...
      <v-chart
        ...
        "
      />
      <dash-operador-list ref="list_cargo" />
    </div>
    <div class="col-12 col-sm-6">
      <div class="t-font-bold t-text-xl t-text-center t-px-2 t-pt-2">
      ...
      <v-chart
        ...
        "
      />
      <dash-operador-list ref="list_repor" />
    </div>
    <div class="col-12 col-sm-6">
      <div class="t-font-bold t-text-xl t-text-center t-px-2 t-pt-2">
      ...
      <v-chart
        ...
        "
      />
      <dash-operador-list ref="list_oper" />
    </div>
  </m-loading>
</template>
<script setup>
...
import DashOperadorList from './components/DashOperadorList.vue'
...
function setBarOption(draw, res, key) {
  ...
  Object.entries(order).forEach(([k, v]) => {
    area.push(k)
    info.alto.push(Math.round((v.alto * 1000) / v.total) / 10)
    info.medio.push(Math.round((v.medio * 1000) / v.total) / 10)
    info.bajo.push(Math.round((v.bajo * 1000) / v.total) / 10)
  })
...
function operador_click(handle, props) {
  console.log('click area', handle, props)
  handle?.reload(props)
}
function reload() {
  main.value.get('/fatiga/admin/dashboard', (b) => {
    console.log(b)
    // eliminar datos repetidos
    setBarOption(opt_area, b.area, 'area')
