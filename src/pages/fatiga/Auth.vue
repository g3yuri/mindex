		...
		<div>
			<m-loading v-if="!message" ref="main">
				<div class="row q-col-gutter-md" style="max-width: 400px; margin: auto">
					...
					<div class="col-12">
						<q-input outlined label="Nombres y apellidos" v-model="info.nombres" />
					</div>
					<div class="col-12 text-center">
						<q-btn unelevated color="primary" @click="registrar" label="Verificar registro" />
					</div>
				</div>
			</m-loading>
			<div v-else class="col-12 text-center t-font-bold t-text-2xl">
				<div class="t-py-5">{{message}}</div>
				<q-btn label="Reintentar" @click="message=null" rounded unelevated color="primary"/>
			</div>
		</div>
	</q-page>
</template>
		  ...
		  const registrar = ()=>{
			...
			main.value.post('/fatiga/registro',info,b=>{
				...
				if (b.op=='registrado') {
					...
					message.value = 'Respuesta desconocida:'+new String(b.op)
				}
			})
		  }
		  return {
