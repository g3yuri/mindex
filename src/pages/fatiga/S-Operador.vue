<template>
  <m-loading ref="main" class="t-m-4">
    ...
    <div class="row q-col-gutter-md">
      <div class="col-4" :key="index" v-for="(el, index) in list">
        <div
          :class="`t-rounded-lg t-p-2 t-h-14 t-text-center ${el.class}`"
          @click="
            router.push(`/fatiga/operador/${route?.params?.dni}/${el.fecha}`)
          "
        >
          <div>{{ el.fecha }}</div>
