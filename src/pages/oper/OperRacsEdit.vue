<template>
	<m-loading ref="main">
		...
		<div v-else class="row q-col-gutter-md q-pa-md">
			...
			<div
				:class="
					't-w-3/4 t-rounded-sm t-p-1 t-border-l-4 ' +
					obs.riesgo_class
				"
			>
				<div>{{ obs.info }}</div>
				...
				<div class="t-flex t-justify-between t-w-full">
					<div
						v-if="obs.user_id > 0"
						class="t-text-gray-500 t-text-xs"
					>
		...
		>
			<div class="col-12">
				<q-checkbox
					v-model="levantado"
					label="Fue levantado la observacion"
				/>
			</div>
...
const ob_causas = computed(() => causas_raw.value?.map(e=>({label:e.descripcion, value: e.id})));
