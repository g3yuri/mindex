<template>
	<div class="t-p-2">
		...
		<m-loading ref="main">
			<template v-if="tag=='reporte'">
				<div class="t-w-full t-flex">
					<div :class="`t-px-1 t-h-14 t-w-2/5 t-text-center t-flex t-items-center t-justify-center`">
						<div class="t-font-bold t-text-xl t-text-center">Areas</div>
					</div>
					<div :class="`t-px-1 t-h-14 t-w-1/5 t-text-center`" v-for="el in dias">
						<div class="t-border-2 t-rounded-lg t-border-gray-300">
							<div class="t-font-bold t-text-xl t-text-center">{{el.dia}}</div>
							<div class="t-uppercase t-text-gray-400 t-text-center">{{el.mes}}</div>
						</div>
					</div>
				</div>
				<div class="t-w-full t-flex" v-for="el in areas">
					<div class="t-p-1 t-h-14 t-w-2/5" >
						<div class="t-font-bold t-text-xs t-h-full t-rounded-lg t-border-2 t-border-gray-300 t-flex t-items-center t-p-1">{{el.area}}</div>
					</div>
					<div :class="`t-p-1 t-h-14 t-w-1/5 t-text-center`" v-for="dy in dias"
						dclick="router.push(`/oper/obs/new?fecha=${dy.fecha}&area=${el.area}`)"
						@click="router.push(`/inspec/area?area_rep=${el.area}&fecha=${dy.fecha}`)">
			          	<div v-if="el[dy.fecha]?.clase" :class="el[dy.fecha].clase" >{{el[dy.fecha].cant}}</div>
			        </div>
				</div>
			</template>
			<template v-if="tag=='pendiente'">
				
		<div v-for="el in pendiente" class="t-w-full t-flex t-py-2"
			@click="editar(el.id)">
		...
		<div v-for="el in levantado" class="t-w-full t-flex t-py-2"
			@click="editar(el.id)">
			<div class="t-w-2/6 t-pr-1">
				<div class="t-flex t-items-center">
					<div class="t-font-bold t-text-lg t-pr-2">{{el.nivel}}</div>
			...
			main.value.get('/labor/super/resumen',b=>{
				...
				b.areas.forEach(e=>{
					for(var k of Object.keys(e)) {
						...
						e[k].clase = 't-rounded-lg t-w-full t-h-full t-flex t-justify-center t-items-center t-text-lg '+(e[k].cant ? 't-bg-emerald-400' : 't-bg-gray-300')
					}
				})
				areas.value = b.areas
