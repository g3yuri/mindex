<template>
	<m-loading ref="main" class="t-m-4">
		<div class="t-flex t-flex-row t-justify-between t-items-center t-pb-4">
			<div class="t-flex-1">
				<div v-for="el in Object.keys(payload)" class="t-font-bold t-uppercase">
					{{el}}: {{payload[el]}}
				</div>
			</div>
			<q-btn label="Agregar" @click="agregar" icon="add" unelevated color="primary"/>
		</div>
		<div class="row q-col-gutter-lg">
			<div v-for="el in list" class="col-xs-12 col-md-6 flex" >
				<div class="t-w-2/6 t-pr-1">
					...
					<div :class="'t-w-full t-h-24 t-overflow-hidden t-rounded-lg t-flex t-justify-center t-items-center '+(el.rel?'':'t-border-2')">
					    <div v-if="el.rel" class="images" v-viewer="{toolbar: false, navbar: false, title: false,}">
				...
				<div :class="'t-w-4/6 t-rounded-sm t-p-1 t-border-l-4 '+el.riesgo_class" @click="editar(el.id)">
					<div>{{el.info}}</div>
...
<script setup>
	...
	function agregar() {
		let {zona, turno, fecha} = payload
		router.push(`/inspec/obs/new?zona=${zona??''}&turno=${turno??''}&fecha=${fecha??''}`)
	}
	
	onMounted(()=>{
		let pm = route.params
		let {nivel, area, zona, area_rep, fecha} = route.query
