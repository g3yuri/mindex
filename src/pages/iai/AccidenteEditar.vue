      ...
      <div>
        <div class="row q-col-gutter-sm">
          ...
          <q-select
            ...
            dense
            outlined
            v-model="info.potencial"
            :options="helper.potencial"
          />
          <div class="col-sm-2 col-xs-4 t-font-bold t-flex t-items-center">
            FECHA
          </div>
          ...
          <q-input
            ...
            v-model="info.lugar"
          />
          <div class="col-12 t-font-bold t-flex t-items-center">
            DESCRIPCION
          </div>
          <q-input
            type="textarea"
          ...
          />
          <div class="col-sm-3 col-xs-12 t-font-bold t-flex t-items-center">
            DIAGNOSTICO
          </div>
          <q-input
  ...
  methods: {
    reload() {
      this.$refs.main.get('/iai/helper', (b) => {
        const opt = []
        Object.keys(b.tipos).forEach(function (value, index) {
          opt.push({ value, label: b.tipos[value] })
        })
        const opt_danios = []
        Object.keys(b.danios).forEach(function (value, index) {
        ...
        this.helper.areas.forEach((value, index) => {
          const pp = []
          Object.keys(b.areas[value]).forEach(function (val, ind) {
    ...
    exp_eliminar(payload) {
      this.$q
        ...
        .onOk(() => {
          payload.acc_id = this.acc_id
          this.$m.api.delete(
            `/rrhh/experiencia?acc_id=${this.acc_id}&id=${payload.id}`,
            (b) => {
              payload.show = false
              this.experiencia = b.experiencia
            },
    ...
    upload_foto(file) {
      this.$m.api.post(
        '/rrhh/upload/adjunto',
        { file, prefix: 'foto', acc_id: this.acc_id },
        (b) => {
    ...
    upload_policial(file) {
      this.$m.api.post(
        '/rrhh/upload/adjunto',
        { file, prefix: 'policial', acc_id: this.acc_id },
        (b) => {
    ...
    upload_penal(file) {
      this.$m.api.post(
        '/rrhh/upload/adjunto',
        { file, prefix: 'penal', acc_id: this.acc_id },
        (b) => {
    ...
    upload_emo(file) {
      this.$m.api.post(
        '/rrhh/upload/adjunto',
        { file, prefix: 'emo', acc_id: this.acc_id },
        (b) => {
